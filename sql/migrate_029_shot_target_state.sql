...
SET @clauses := CONCAT_WS(', ',
    ...
    IF((SELECT COUNT(*)=0 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='ktp_shot_events' AND COLUMN_NAME='net_lerp'), 'ADD COLUMN net_lerp SMALLINT DEFAULT NULL COMMENT ''client ex_interp ms for the packet this shot arrived in -- how far into the past the client rendered''', NULL),
    IF((SELECT COUNT(*)=0 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='ktp_shot_events' AND COLUMN_NAME='net_dropped'), 'ADD COLUMN net_dropped SMALLINT DEFAULT NULL COMMENT ''commands the engine never received in that packet -- above MAX_DROPPED_CMDS it replays lastcmd, so a shot can resolve against fabricated input''', NULL),
    IF((SELECT COUNT(*)=0 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='ktp_shot_events' AND COLUMN_NAME='net_backup'), 'ADD COLUMN net_backup SMALLINT DEFAULT NULL COMMENT ''command backup depth for that packet''', NULL),
    IF((SELECT COUNT(*)=0 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='ktp_shot_events' AND COLUMN_NAME='net_cmds'), 'ADD COLUMN net_cmds SMALLINT DEFAULT NULL COMMENT ''commands in that packet''', NULL));
